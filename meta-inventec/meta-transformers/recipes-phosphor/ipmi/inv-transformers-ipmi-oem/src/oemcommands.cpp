...
/**
 *   Function to read/write register value by calling devmem
 *   If read is false, means write, value should store to address.
 *   If read is true, value would return the value in address.
 **/
static void devmem(uint32_t address, uint32_t* value, bool read)
{
    void *map_base, *virt_addr;
    unsigned page_size, mapped_size, offset_in_page;
    int fd;

    fd = open("/dev/mem", read?(O_RDONLY | O_SYNC):(O_RDWR | O_SYNC));
    if(fd == -1)
    {
        throw "devmem open fail";
    }

    mapped_size = page_size = getpagesize();

    offset_in_page = (unsigned)address & (page_size - 1);
    if (offset_in_page + 8*sizeof(uint32_t) > page_size) {
    ...
    map_base = mmap(NULL,
            mapped_size,
            read?(PROT_READ):(PROT_READ | PROT_WRITE),
            MAP_SHARED,
            fd,
...
/**
 *  @brief Function of getting boot flash ID
 *  @brief NetFn: 0x30, Cmd: 0xb7
 *
 *  @return Byte 1: Completion Code.
 *          Byte 2: Boot ID - 01h Primary, 02h Backup.
 **/
ipmi::RspType<uint8_t> ipmiOemGetBootFlash()
{
    uint8_t bootID;
    uint32_t status;

    try{
        devmem(inv::fmcWDT2ControlReg, &status, true);
    }
    catch(std::exception& e){
        phosphor::logging::log<phosphor::logging::level::ERR>(
                        "Fail to get BMC boot flash status",
                        phosphor::logging::entry("ERROR=%s", e.what()));
        return ipmi::responseUnspecifiedError();
    }

    /* FMC_WDT2 status boot code source : bit 4*/
    if( status & 0x10)
    {
        bootID = 2;
    }
    ...
    return ipmi::responseSuccess(bootID);
}

/**
 *  @brief Function of setting boot flash, would trigger HW WDT timeout immediately
 *  @brief NetFn: 0x30, Cmd: 0xb8
 *
 *  @param[in] Boot number - 01h Primary, 02h backup.
 *
 *  @return Byte 1: Completion Code.
 **/

ipmi::RspType<> ipmiOemSetBootFlash(uint8_t bootID)
{
    uint8_t currentBootId;
    uint32_t status;
    uint32_t writeValue;
    ...
    {
        /*Set FMC_WDT2 timeout to 30 sec */
        writeValue = 300;
        try{
            devmem(inv::fmcWDT2ReloadReg, &writeValue, false);
        }
        catch(std::exception& e){
            phosphor::logging::log<phosphor::logging::level::ERR>(
                            "Fail to set wdt2 timeout",
                            phosphor::logging::entry("ERROR=%s", e.what()));
            return ipmi::responseUnspecifiedError();
        }
        /*Trigger FMC_WDT2 timeout */
        writeValue = 0x01;
        try{
            devmem(inv::fmcWDT2ControlReg, &writeValue, false);
        }
        catch(std::exception& e){
            phosphor::logging::log<phosphor::logging::level::ERR>(
                            "Some error in start wdt2",
                            phosphor::logging::entry("ERROR=%s", e.what()));
            return ipmi::responseUnspecifiedError();
        }
    }
...
static void registerOEMFunctions(void)
{
    ...
    registerOemCmdHandler(inv::netFnOem30, inv::cmdsNetFnOem30::cmdGetBootFlash,
                            Privilege::Admin, ipmiOemGetBootFlash);

    registerOemCmdHandler(inv::netFnOem30, inv::cmdsNetFnOem30::cmdSetBootFlash,
                            Privilege::Admin, ipmiOemSetBootFlash);
