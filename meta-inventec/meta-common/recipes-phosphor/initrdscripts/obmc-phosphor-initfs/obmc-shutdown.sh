...
# Inventec - prevent BMC and BMC2 update in the same time
bmc_image=/run/initramfs/image-bmc
bmc2_image=/run/initramfs/image-bmc2

if [ -f $bmc_image ]; then
    if [ -f $bmc2_image ]; then
        echo "Both BMC image found, remove BMC2 image"
        rm $bmc2_image
    fi
fi
